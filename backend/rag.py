...
async def retrieve_context(query: str, top_k: int = 5, context_radius: int = 1) -> dict:
    ...
    with vector_search_duration_seconds.time():
        # Run RPC calls concurrently
        verses_coro = supabase_async.rpc("match_verses", {
            "query_embedding": query_emb,
            "match_count": top_k
        }).execute()
        
        footnotes_coro = supabase_async.rpc("match_footnotes", {
            "query_embedding": query_emb,
            "match_count": top_k
        }).execute()
        
        subtitles_coro = supabase_async.rpc("match_subtitles", {
            "query_embedding": query_emb,
            "match_count": top_k
        }).execute()
